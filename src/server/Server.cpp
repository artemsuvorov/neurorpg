#include "Precompiled.h"
#include "Server.h"

#include <asio.hpp>
#include <atomic>
#include <thread>


using tcp = asio::ip::tcp;


static void PrintError(const std::error_code& error)
{
	printf("Error %u: %s\n", error.value(), error.message().c_str());
}


namespace Neuro::Net {

	struct Server::Session final : public std::enable_shared_from_this<Session>
	{
	public:
		Session(tcp::socket&& socket) : m_Socket(std::move(socket)) {}

		void Run();

		asio::awaitable<void> EchoAsync();

	private:
		tcp::socket m_Socket;
		char m_Buffer[1024] = {0};
	};


	void Server::Session::Run()
	{
		const auto Coroutine = [self = shared_from_this()]() -> asio::awaitable<void> {
			co_await self->EchoAsync();
		};

		asio::co_spawn(m_Socket.get_executor(), Coroutine, asio::detached);
	}


	asio::awaitable<void> Server::Session::EchoAsync()
	{
		// Keep session alive while coroutine runs.
		auto self = shared_from_this();
		asio::error_code error;

		while (true)
		{
			std::size_t length = co_await m_Socket.async_read_some(asio::buffer(m_Buffer),  //
			                                                       asio::redirect_error(asio::use_awaitable, error));
			if (error)
				break;

			co_await asio::async_write(m_Socket, asio::buffer(m_Buffer, length),
			                           asio::redirect_error(asio::use_awaitable, error));
			if (error)
				break;
		}

		PrintError(error);
	}

}  // namespace Neuro::Net


namespace Neuro::Net {

	struct Server::Acceptor final
	{
	public:
		Acceptor(asio::io_context& context) : m_Context(context), m_Acceptor(context) {}

		bool Open(uint16_t port, asio::error_code& error);
		void Close();
		void Accept();

	private:
		asio::io_context& m_Context;
		tcp::acceptor m_Acceptor;
	};


	bool Server::Acceptor::Open(uint16_t port, asio::error_code& error)
	{
		m_Acceptor.open(asio::ip::tcp::v4(), error);
		if (error)
			return false;

		m_Acceptor.set_option(asio::ip::tcp::acceptor::reuse_address(true), error);
		if (error)
			return false;

		m_Acceptor.bind(asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port), error);
		if (error)
			return false;

		m_Acceptor.listen(asio::socket_base::max_listen_connections, error);
		return !error;
	}


	void Server::Acceptor::Close()
	{
		asio::error_code error;
		m_Acceptor.close(error);
		if (error)
			PrintError(error);
	}


	void Server::Acceptor::Accept()
	{
		m_Acceptor.async_accept([this](asio::error_code error, asio::ip::tcp::socket socket) {
			if (error)
			{
				PrintError(error);
				return;
			}

			std::make_shared<Session>(std::move(socket))->Run();
			Accept();
		});
	}

}  // namespace Neuro::Net


namespace Neuro::Net {

	struct Server::Impl final
	{
	public:
		Impl(Server::Config&& config) : m_Config(std::move(config)), m_Acceptor(m_Context) {}
		~Impl();

		void Start();
		void Stop();

		bool IsRunning() const;

	private:
		asio::io_context m_Context;
		Server::Config m_Config;
		Server::Acceptor m_Acceptor;
		std::atomic<bool> m_IsRunning = false;
		std::thread m_Thread;
	};


	Server::Impl::~Impl()
	{
		Stop();
	}


	void Server::Impl::Start()
	{
		if (m_IsRunning.load(std::memory_order::acquire))
			return;

		m_Context.restart();

		const uint16_t port = m_Config.Port;
		asio::error_code error;
		if (!m_Acceptor.Open(port, error))
		{
			PrintError(error);
			return;
		}

		printf("Server listening on port %u ...\n", port);
		m_IsRunning.store(true, std::memory_order::release);
		m_Acceptor.Accept();

		m_Thread = std::thread([this]() {
			m_Context.run();
		});
	}


	void Server::Impl::Stop()
	{
		if (!m_IsRunning.load(std::memory_order::acquire))
			return;

		m_Context.stop();
		m_Acceptor.Close();
		if (m_Thread.joinable())
			m_Thread.join();
		m_IsRunning.store(false, std::memory_order::release);
	}


	inline bool Server::Impl::IsRunning() const
	{
		return m_IsRunning.load(std::memory_order::acquire);
	}

}  // namespace Neuro::Net


namespace Neuro::Net {

	Server::Server(Config&& config) : m_Impl(std::make_unique<Impl>(std::move(config))) {}


	Server::~Server() = default;


	void Server::Start()
	{
		m_Impl->Start();
	}


	void Server::Stop()
	{
		m_Impl->Stop();
	}


	bool Server::IsRunning() const
	{
		return m_Impl->IsRunning();
	}

}  // namespace Neuro::Net
